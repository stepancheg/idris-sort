...
export
gtFirstNotLTE : {t : TotalOrder Nat} -> TotalOrder.gt t x y -> Not (LTEListNat t (x :: xs) (y :: ys))
...
private
notLteNotEmptyEmpty : {t : TotalOrder Nat} -> Not (LTEListNat t (x :: xs) [])
...
private
fromLTERecEq : {t : TotalOrder Nat} -> eq t x y -> (LTEListNat t (x :: xs) (y :: ys)) -> LTEListNat t xs ys
...
isLTEListNat : {t : TotalOrder Nat} -> (xs, ys : List Nat) -> Dec (LTEListNat t xs ys)
...
private
lteListNatTransitiveHelp : {t : TotalOrder Nat} -> xs = [] -> LTEListNat t xs []
lteListNatTransitiveHelp Refl = LTEListNatZero

private
lteListNatTransitive : {t : TotalOrder Nat} -> LTEListNat t xs ys -> LTEListNat t ys zs -> LTEListNat t xs zs
...
private
lteListNatAntisymmetricHelp : {t : TotalOrder Nat}
...
private
lteListNatAntisymmetric : {t : TotalOrder Nat} -> Not (LTEListNat t xs ys) -> LTEListNat t ys xs
lteListNatAntisymmetric not_lte = lteListNatAntisymmetricHelp not_lte

export
totalOrderListLte : TotalOrderLite (List Nat)
totalOrderListLte =
    TotalOrderLite_mk (LTEListNat totalOrderNat) isLTEListNat lteListNatTransitive lteListNatAntisymmetric
